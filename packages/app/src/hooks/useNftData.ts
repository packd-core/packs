...
export function useNftData({tokenId, address, chainId}: { tokenId: bigint, address: Address, chainId?: number }) {
    const {data: tokenUri, isLoading, isError} = useErc721TokenUri({address, args: [tokenId], chainId})
    const {data: uri, isLoading: isErc1155Loading, isError: isErc1155Error} = useContractRead({
        abi: [{
        ...
        functionName: 'uri', address, args: [tokenId], chainId
    })
    ...
    useEffect(() => {
        if (data) {
            const url = data.replace('ipfs://', 'https://ipfs.io/ipfs/').replace('/ipfs/ipfs', '/ipfs')
            setIsNftDataLoading(true)
            fetch(`/api/nftData?url=${url}`, {mode: 'no-cors'}).then((response) => {
                if (!response.ok) {
                    return;
                }
