...
export default function AddNftModal({
  ...
  const queryNftAddress = useCallback(
          ...
          res.map(async (tokenId) => {
            return pc
              ...
              .then((tokenURI) => {
                ...
                const tokenURIReplaced = replaceIpfsUrl(tokenURI);
                return fetch(tokenURIReplaced)
                  .then((res) => res.json())
                  .then((res) => {
                    return {
                      name,
  ...
  useEffect(() => {
    const list = (unfilteredNftList ?? []).filter((nft) =>
  ...
  return (
    <Modal
      panelClassNames="max-w-md md:max-w-lg"
      render={(closeModal) => (
        <div className="flex flex-col bg-[#202020] rounded-3xl gap-2 text-white p-4">
          <div className="flex justify-between">
            ...
            <button
              className="hover:text-primary-500 pl-2"
              onClick={closeModal}
            >
              <BsX />
            </button>
            ...
            {isManual && (
              <>
                ...
                <div className="relative">
                  ...
                  <input
                    className="text-right w-full pl-12 text-xs py-2"
                    onChange={(e) => {
                      const value = e.target.value;
                      if (isAddress(value)) {
                ...
                <div className="flex justify-between">
                  ...
                  {!isOwnerLoading && !!nftAddress && tokenId !== undefined && (
                    <span
                      className={clsxm(
                        !hasToken && "text-red-500",
                        "text-xs font-semibold"
                      )}
                    >
                      Available: {hasToken ? "yes" : "You don't own this token"}
                    </span>
                  )}
                </div>
                <input
                  className={clsxm(
                    "text-right",
                    ((tokenId ?? -1) < 0 || !hasToken) && "text-red-500"
                  )}
                  onChange={(e) => {
                    const value = e.target.value;
...
function NftSearchItem({
  ...
  const mediaUrl = replaceIpfsUrl(item.media?.original_media_url ?? "");

  const isVideo = item.media?.mimetype?.includes("video") ?? false;
  return (
    <div className="hover:border-primary-50 border-transparent border rounded-lg overflow-clip">
      <button
        onClick={onClick}
      ...
      >
        ...
        <div className="flex flex-col gap-2 grow justify-end w-full">
              ...
              <>
                {isVideo ? (
                  ...
                  <img
                    ...
                    alt={item.name ?? ""}
                    className={
                      "object-contain w-full h-full text-center text-sm"
                    }
                  />
                )}
              </>
          ...
          <div className="w-full flex px-2 pb-2">
            <span className="text-xs text-gray-400 text-ellipsis overflow-hidden shrink">
              #{item.token_id}
            </span>
          </div>
        </div>
