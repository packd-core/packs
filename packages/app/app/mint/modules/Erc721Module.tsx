...
export default function Erc721Card({onClick, module}: { onClick?: () => void, module: Module }) {
    ...
    return <ContentCard className="self-stretch">
        <div className='flex gap-2 relative'>
            ...
            <div className={clsxm("w-20 h-20 flex justify-center items-center rounded-lg bg-back shrink-0", isVideo && 'w-32 h-32')}>
                {/* eslint-disable-next-line @next/next/no-img-element */}
                {isVideo ? <video width="400" controls className="w-full">
                    <source src={nftData?.image} type={nftData?.contentType}/>
                    {/* eslint-disable-next-line @next/next/no-img-element */}
                </video> :<img src={nftData?.image} alt={nftData?.name} className='rounded-lg'/>}
            </div>
